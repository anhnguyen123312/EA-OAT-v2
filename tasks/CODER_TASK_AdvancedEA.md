# CODER_TASK - AdvancedEA v1.00 (SMC/ICT Baseline)

**Version:** 1.01 (Revised - Post-Review Fixes)
**Date:** 2026-02-17
**Author:** Technical Analyst
**Status:** ✅ APPROVED (All CRITICAL fixes applied)
**Iteration:** 1 (Phase 1 - SMC Core Baseline)

---

## 📋 Executive Summary

This document provides **complete MQL5 technical specifications** for implementing **AdvancedEA v1.00** - a Smart Money Concepts (SMC/ICT) Expert Advisor for XAUUSD M5 trading.

**Source Documents:**
- `brain/implementation_plan.md` (PM's plan - 352 lines)
- `brain/advanced_smc_research.md` (SMC research - 879 lines)
- `brain/strategy_research.md` (Strategy overview)
- `/EA-OAT-reference/archive/v2/Experts/V2-oat.mq5` (Reference implementation)

**Performance Targets (ALL must be met):**
- Win Rate: ≥ 80% (stretch goal: 90%)
- Risk:Reward: ≥ 1:2
- Trades/Day: ≥ 10
- Max Drawdown: < 10%
...
├── code/
│   ├── experts/
│   │   └── AdvancedEA.mq5                # Main EA file (ALL IN ONE FILE)
│   └── include/                          # For future iterations (Phase 2+)
├── config/
│   └── active/
│       └── autobacktest.ini              # Backtest configuration
└── results/
    └── YYYY-MM-DD_AdvancedEA_*.csv       # OnTester() CSV export
```

**IMPORTANT:** For Phase 1, implement **ALL code in a single .mq5 file** (no separate include files). This simplifies debugging and ensures compilation success. Modularization can be done in Phase 2 if targets are met.
...
```mql5
AdvancedEA.mq5 structure:
├── File header (#property, copyright, version)
├── Input parameters (grouped by function)
├── Data structures (structs, enums)
├── Global variables
├── Detector classes (BOS, Sweep, OB, FVG, MTF)
├── Scorer class
├── Risk Manager class
├── Trade Manager class
├── OnInit() - Initialize all components
├── OnDeinit() - Cleanup
...
**Why single file?**
- Faster compilation (no include path issues)
- Easier debugging (all code visible)
- Proven to work (SimpleEA validated this)
- Can refactor to modular structure in Phase 2 if needed
...
struct SetupCandidate {
    ...
    // Signals (pointers not needed - use global signals)
    int      bosAge;
    int      sweepAge;
    int      obStrength;
    int      fvgAge;
...
## 🔧 Helper Functions (Add to AdvancedEA.mq5 before detector classes)

**CRITICAL:** MQL5 indicator functions return HANDLES, not values. Use these helpers:
...
**Purpose:** Detect trend direction changes by identifying swing breaks
...
public:
    ...
    BOSSignal DetectBOS() {
        ...
        if(m_lastSwingHigh.price > 0) {
            ...
            double atr = GetATR(_Symbol, _Period, 14, 1);  // FIXED: Use helper function
...
### 2. Sweep Detector (Liquidity Sweep)

**Purpose:** Detect stop hunts where price sweeps a level then reverses
...
private:
    bool IsBullishOB(int index) {
        ...
        double atr = GetATR(_Symbol, _Period, 14, index);  // FIXED: Use helper function
    ...
    int CalculateOBStrength(OrderBlock &ob) {
        ...
        // NOTE: Full implementation per advanced_smc_research.md would add:
        // - Fractal S/R bonus +3
        // - Volume spike bonus +2
        // - 50% fib bonus +2
        // - Long wick bonus +1
        // This simplified version prioritizes age/freshness which is most reliable
    }
};
```

### 4. FVG Detector (Fair Value Gap)
...
public:
    ...
    void DetectFVGs() {
        ...
        for(int i = 2; i < 20; i++) {
            ...
            double atr = GetATR(_Symbol, _Period, 14, i);  // FIXED: Use helper function
...
public:
    int GetMTFBias() {
        // FIXED: Use GetMA() helper function instead of iMA() directly
...
**Purpose:** Combine all detector outputs into single entry score
...
public:
    ...
    // ADDED: Trigger Candle Detection (Design Decision 14)
    bool HasTriggerCandle(int direction) {
        double atr = GetATR(_Symbol, _Period, 14, 1);

        // Scan last 4 bars (0-3) for trigger candle
        for(int i = 0; i < 4; i++) {
            double open = iOpen(_Symbol, _Period, i);
            double close = iClose(_Symbol, _Period, i);
            double body = MathAbs(close - open);

            // Check if candle matches direction and has sufficient body
            if(direction == 1) {
                // Bullish trigger: close > open, body >= 0.30 ATR
                if(close > open && body >= 0.30 * atr) {
                    return true;
                }
            } else if(direction == -1) {
                // Bearish trigger: close < open, body >= 0.30 ATR
                if(close < open && body >= 0.30 * atr) {
                    return true;
                }
            }
...
private:
    void CalculateEntryLevels(SetupCandidate &candidate, BOSSignal &bos,
                               OrderBlock &ob_bull, OrderBlock &ob_bear,
                               FVGSignal &fvg_bull, FVGSignal &fvg_bear) {
        double atr = GetATR(_Symbol, _Period, 14, 0);  // FIXED: Use helper function
...
void OnTick() {
    ...
    // ADDED: Trigger candle requirement (Design Decision 14)
    ...
    // FIXED: Added MTF bias to trade comment
...
double OnTester() {
    ...
    double winRate = TesterStatistics(STAT_TRADES) > 0
                   ? (TesterStatistics(STAT_WIN_TRADES) / TesterStatistics(STAT_TRADES)) * 100.0
                   : 0;

    // FIXED: R:R calculation - average winner / average loser
...
### Phase 1: Single-File Implementation

- [ ] **File Setup**
  - [ ] Create `code/experts/AdvancedEA.mq5`
  - [ ] Add all input parameters (grouped)
  - [ ] Define all structs and enums

- [ ] **Detector Classes**
  - [ ] CBOSDetector (complete with Init, Update, DetectBOS, UpdateBOSRetest)
  - [ ] CSweepDetector (complete with Init, Update, DetectSweep)
  - [ ] COrderBlockDetector (complete with Init, Update, DetectOrderBlocks)
  - [ ] CFVGDetector (complete with Init, Update, DetectFVGs)
  - [ ] CMTFBiasAnalyzer (GetMTFBias method)

- [ ] **Scorer & Risk**
  - [ ] CConfluenceScorer (BuildCandidate, CalculateScore, CalculateEntryLevels)
  - [ ] CRiskManager (CalculateLotSize, CheckDailyMDD, ManagePositions, ApplyBreakeven)

- [ ] **Main EA**
  - [ ] OnInit() - instantiate all objects
  - [ ] OnDeinit() - cleanup
  - [ ] OnTick() - main loop
  - [ ] OnTester() - CSV export
...
- [ ] **Backtest Config**
  - [ ] Update `config/active/autobacktest.ini`
  - [ ] Expert=AdvancedEA (name only, no path)
  - [ ] Symbol=XAUUSD, Period=5
  - [ ] Date: 2023.01.01 to 2025.12.31

- [ ] **Execute & Validate**
  - [ ] Run backtest via Wine
  - [ ] CSV exported to Common/Files
  - [ ] Copy CSV to `results/`
  - [ ] Git commit with message: "Iteration 1: AdvancedEA SMC baseline"
...
- ALL code in `AdvancedEA.mq5` (no separate .mqh files)
- Simplifies compilation and debugging
- Can refactor later if targets are met

### 2. Compilation Tips
- Save frequently and compile incrementally
- Fix errors immediately as they appear
- Test each detector class separately if possible

### 3. Backtest Config
- `Expert=AdvancedEA` (NOT `Expert=Experts\AdvancedEA.ex5`)
- Must include Login and Server parameters
- Use Model=1 (1-min OHLC) for faster testing

### 4. CSV Export
- Use `FILE_COMMON` flag (mandatory)
- Path: `~/.wine/drive_c/users/$USER/AppData/Roaming/MetaQuotes/Terminal/Common/Files/`
- Use `FILE_ANSI` for UTF-8 encoding

### 5. Score Threshold
- Start with 100 points
- If < 10 trades/day → lower to 90
- If > 20 trades/day → raise to 120
...
**Backtest Execution:**
- ✅ Runs 3-year date range without crash
- ✅ Completes in < 30 minutes

**CSV Export:**
- ✅ File exists in Common/Files
- ✅ Contains all standard metrics
- ✅ Win Rate, R:R, Trades, Max DD all present

**Performance (Stretch Goals):**
- Win Rate: 80%+ (90% stretch)
- R:R: 2.0+
- Trades/Day: 10+
- Max DD: < 10%
...
### Version 1.01 - 2026-02-17 (Post-Review Fixes)

**Status:** ✅ REVISED - All Critical Issues Fixed

**Changes Made (responding to Reviewer feedback in brain/review_log.md):**

#### CRITICAL Fixes:
1. ✅ **Fixed MQL5 API Usage** - Added GetATR() and GetMA() helper functions, updated all iATR()/iMA() calls to use handle-based pattern
2. ✅ **Fixed OB Strength Calculation** - Changed base score from 5 to 8, implemented gradual penalties to allow Strong OBs (>=7)
3. ✅ **Added Trigger Candle Requirement** - Implemented HasTriggerCandle() function per Design Decision 14 (body >= 0.30 ATR)
4. ✅ **Fixed R:R Calculation** - Corrected OnTester() formula to use average winner / average loser

#### MAJOR Fixes:
5. ✅ **Added MTF to Trade Comment** - Comment now includes MTF bias: "SMC:XXX|BOS:Y|SW:Y|OB:Y|FVG:Y|MTF:Y"

**Remaining Items (Recommended, not blocking):**
- 📝 Win Rate target inconsistency (targets.md vs implementation_plan.md) - Team Lead to resolve
- 📝 SMC-specific metrics in OnTester() - Can be added in future iteration if needed
- 📝 Entry/SL calculation logic - Current implementation valid, can optimize later

**Compilation Safety:** All indicator calls now use proper handle-based pattern. Code should compile with ZERO errors.

---

**Status:** ✅ READY FOR CODER (All critical issues resolved)
**Next:** Coder implements → Compiles → Backtests → CSV export
**Timeline:** 3-5 days (code + compile + backtest + analyze)

---

*Generated by Technical Analyst*
*Date: 2026-02-17*
*Version: 1.01 (Revised)*
*Based on PM's implementation_plan.md + Reviewer feedback*
